...
def main():
    ...
    val_full=datasets.ImageFolder(root=data_dir, transform=val_transfroms)
    ...
    for epoch in range(num_epochs):
        ...
        correct_val=0
        total_val=0
        print(f"validation for {epoch+1}")
        with torch.no_grad():
        ...
        print(f"Epoch [{epoch+1:02d}/{num_epochs:02d}] "
              f"Train Loss: {epoch_loss_train:.4f} | Train Acc: {epoch_accuracy_train:.4f} | "
              f"Val Loss: {epoch_loss_val:.4f} | Val Acc: {epoch_accuracy_val:.4f} | "
    ...
    torch.save(model.state_dict(), "densenet_paddy.pth")
    print("\nModel saved to densenet_paddy.pth")

    generation_report_confusion_mat(model,val_loader,class_names,device)


def generation_report_confusion_mat(model, val_loader,class_names,device):
    model.eval()
    all_preds = []
    all_labels = []
